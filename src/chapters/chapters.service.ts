...
export class ChaptersService {
    constructor(@InjectModel(Chapter) private chapterRepository: typeof Chapter, @InjectModel(Paragraph) private paragraphRepository: typeof Paragraph) {}
    ...
    async update(dto: ChapterUpdateDto){
        const [numberOfAffectedRows, [updateChapters]] = await this.chapterRepository.update(dto, {
            where: {id: dto.chapter_id},
            returning: true
        })
    ...
    async remove(dto: RemoveParagraphDto) {
        const chapter = await this.chapterRepository.findByPk(dto.chapter_id, {include: Paragraph });
        if (!chapter) {
            throw new NotFoundException('Chapter not found!');
        }
