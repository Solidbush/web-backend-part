...
export class LessonsService {
  ...
  async getLesson(lesson_id: number) {
    return await this.lessonRepository.findAll({
      where: { id: lesson_id },
      include: [
        { model: Task, include: [{ model: Question }, { model: Problem }] },
        { model: Chapter, include: [{ model: Paragraph }] },
      ],
    });
  }
  ...
  async update(dto: UpdateLessonDto) {
    const [numberOfAffectedRows, [updatedLessons]] =
      await this.lessonRepository.update(dto, {
